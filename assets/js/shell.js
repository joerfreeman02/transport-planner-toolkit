...
async function refreshShared(){try{const masters=await TPTSharedLibrary.loadAll();renderSharedConnection('Shared masters loaded: '+masters.rail.records.length+' Rail, '+masters.busStops.records.length+' Bus Stop and '+masters.busRoutes.records.length+' Bus Route records.');}catch(error){renderSharedConnection('Shared refresh failed safely: '+error.message+' Local proposals remain unchanged.');}}
