...
The representative stop is selected after compatible route-direction consolidation. The principal headline pattern is then selected from unique canonical physical journeys at that stop, destination/pattern support, repeated compatible-record support, regularity and activity; route extent is only a secondary tie-breaker. This prevents a longer but lightly supported short-working destination from displacing the evidence-led principal destination. Canonical departures are then deduplicated by physical journey identity at that stop, while raw service records, source lineage, provenance and alternate destinations remain available. The planner summary audit reports every route, row count, direction/corridor identity, representative stop, operator family, headline destination and variant count; routes above an expected production count are reported rather than hidden.
