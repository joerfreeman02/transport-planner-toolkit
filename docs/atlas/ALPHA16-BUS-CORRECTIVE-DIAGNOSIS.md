...
The candidate semantic replay produced 71 selected route records and 23 planner rows, but it is not accepted for PAT: 13 was presented as `Towards Epping` despite North Weald/The Talbot pattern evidence; 16/16C still produced a circular headline; 25C still produced two `Towards Waltham Cross` rows; 242 presented only `Towards Waltham Cross` with Potters Bar in a variant note; 310 retained Hertford/Waltham Cross in raw patterns but still produced two `Towards Waltham Cross` planner rows; and 491 presented `Towards Waltham Cross` rather than the required North Middlesex Hospital principal endpoint. The 22 retained live TfL requests all returned HTTP 400/unresolved for the selected request identities. These are reproducible acceptance failures, not grounds for adding new heuristics in this gate.
