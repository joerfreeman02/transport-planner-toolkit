...
The maintenance updater prompts for credentials at runtime and does not persist passwords. It checks the TNDS FTP listing, selects changed England-region archives, stages and validates ZIP candidates, then promotes them transactionally while retaining the previous raw snapshot for rollback. Raw regional archives remain local and ignored. England scope is EA, EM, NE, NW, SE, SW, WM and Y; London remains TfL-primary.
