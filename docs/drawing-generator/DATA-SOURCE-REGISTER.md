...
| Cycle routes | OSM cycleways and bicycle-route relations | Highway cycleways → local `cycle-route`. A bicycle relation is `strategic-cycle` only when it has `network=ncn` or `rcn` **and** a non-empty route `ref`; all other relations are retained as `cycle-review` evidence. | Same | Review-only relation evidence requires a reviewed planner overlay before inclusion as strategic infrastructure. |
...
| Main roads / motorways | OSM highway ways | `highway=trunk/primary` → functional `main-road`; `highway=motorway*` → `motorway`. An A-road statement is made only where an explicit `A…` ref is returned. | Same | Labels preserve returned ref/name (`A406 – North Circular Road`, for example); otherwise the legend says only `MAIN ROAD`. |
