...
function renderPreview() {
  ...
  const result = renderDrawingSvg({ modeId: state.modeId, centerBng: drawingCentre(), site: state.site, sourceFeatures: source.features, overlays: state.overlayStore.list(), sourceStatus: source.status });
  ...
  byId('sheetAttribution').textContent = source.snapshot ? source.snapshot.attribution : 'Manual/reviewed geometry only - external vector source not loaded.';
  ...
  byId('printDrawing').disabled = !scaleMatches;
  if (!scaleMatches) setMessage('drawingStatus', `Print blocked: scale metadata must be 1:${mode.scale.toLocaleString('en-GB')} for ${mode.title}.`, 'error');
  else if (!state.site) setMessage('drawingStatus', `Not issue-ready: no confirmed site boundary. Missing layer evidence: ${missing.join(', ') || 'none'}.`, 'warning');
  else if (missing.length) setMessage('drawingStatus', `Preview ready for review. Required layers without current evidence: ${missing.join(', ')}. Add reviewed overlays or retrieve source data.`, 'warning');
  else setMessage('drawingStatus', 'All configured required layer classes are present. Professional content and route approval are still required.', 'success');
}
...
function acceptSite(input, origin = 'Boundary') {
  try {
    ...
    mapUi.setSite(state.site); persist(); renderPreview();
...
function clearSite() {
  state.site = null; mapUi.clearSite(); persist(); renderPreview();
...
function setLocation(lat, lon, label = 'Coordinate entry') {
  ...
  state.location = { lat, lon, label }; byId('latitudeInput').value = lat.toFixed(6); byId('longitudeInput').value = lon.toFixed(6);
...
renderOverlayRows(); renderSourceStatus(); renderPreview();
setTimeout(() => mapUi.invalidate(), 100);

if (['localhost', '127.0.0.1'].includes(location.hostname)) {
  Object.defineProperty(window, '__DG0_ACCEPTANCE__', {
    value: Object.freeze({
      setMode(id) { if (!DRAWING_MODES[id]) throw new Error('Invalid mode'); state.modeId = id; byId('modeSelect').value = id; populateMetadata(); renderPreview(); },
      setLocation(lat, lon) { setLocation(lat, lon, 'Synthetic acceptance fixture'); },
