...
function featureMarkup(item, project, index) {
  ...
  const style = STYLE[className];
  if (!style) return '';
  const colour = item.properties?.colour || style.stroke || style.fill;
  const point = geometryPoint(item.geometry, project);
  const label = item.properties?.label || item.properties?.routeLabel || item.properties?.name || '';
  if (point) {
    const [x, y] = point;
    ...
    return `<g class="layer-${escapeXml(className)}" data-feature-index="${index}">${symbol}${label ? `<text x="${(x + 6).toFixed(2)}" y="${(y - 5).toFixed(2)}" class="map-label point-label">${escapeXml(label)}</text>` : ''}</g>`;
  }
  ...
  if (!label) return `<g class="layer-${escapeXml(className)}" data-feature-index="${index}">${markup}</g>`;
  const first = item.geometry.type === 'LineString' ? item.geometry.coordinates[Math.floor(item.geometry.coordinates.length / 2)] : null;
  const labelPoint = first ? project(first) : null;
  return `<g class="layer-${escapeXml(className)}" data-feature-index="${index}">${markup}${labelPoint ? `<text x="${labelPoint[0].toFixed(2)}" y="${(labelPoint[1] - 5).toFixed(2)}" class="map-label line-label">${escapeXml(label)}</text>` : ''}</g>`;
}
...
export function legendItemsForDrawing(modeId, sourceFeatures = [], overlays = [], hasSite = false) {
  ...
  const priority = ['site', 'main-road', 'motorway', 'strategic-cycle', 'cycle-route', 'waterway', 'railway', 'station-national-rail', 'station-overground', 'station-underground', 'station-dlr', 'station-tram', 'bus-route', 'community', 'route-to-site', 'route-from-site', 'custom-line', 'custom-point', 'custom-area'];
...
export function renderDrawingSvg({ modeId, centerBng, site = null, sourceFeatures = [], overlays = [], sourceStatus = 'not-loaded' }) {
  ...
  const features = [...visibleSource, ...visibleOverlays, siteFeature(site)].filter(Boolean);
  ...
  const markup = `<svg xmlns="http://www.w3.org/2000/svg" id="drawingSvg" role="img" aria-label="${escapeXml(mode.title)} scale drawing" viewBox="0 0 ${viewWidth} ${viewHeight.toFixed(3)}" preserveAspectRatio="none" data-mode="${modeId}" data-scale="${mode.scale}" data-paper-width-mm="${mode.mapFrameMm.width}" data-paper-height-mm="${mode.mapFrameMm.height}" data-ground-width-m="${extent.groundWidth}" data-ground-height-m="${extent.groundHeight}">
    ...
    <g clip-path="url(#map-clip)">${gridMarkup(extent, viewWidth, viewHeight, mode)}${features.map((item, index) => featureMarkup(item, project, index)).join('')}</g>
  ...
  return { markup, extent, legend: legendItemsForDrawing(modeId, sourceFeatures, overlays, Boolean(site)), scaleBar };
}
...
export function renderFeatureSetForTest(features, extent, modeId) {
  ...
  const project = projector(extent, viewWidth, viewHeight);
  return features.map((item, index) => featureMarkupBound(item, index, project)).join('');
}
