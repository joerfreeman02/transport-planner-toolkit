...
test('assessment uses readable directional frequency controls, not JSON editing',()=>{const h=read('index.html'),j=read('assets/js/railway.js');assert.match(h,/Directional frequencies describe where trains run/i);assert.match(j,/Typical weekday trains per hour by direction or destination/);assert.doesNotMatch(j,/Typical TPH by destination\/direction \(JSON array\)/)});
...
test('build label is visible',()=>assert.ok(read('index.html').includes('RAIL-4.2.0-20260715')));
