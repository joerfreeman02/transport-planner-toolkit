...
export function createAuthoritativeBusTimetableAdapter({ tflAdapter, nationalAdapter, londonCoverage = isGreaterLondonPoint, requestLimit = 20 } = {}) {
  if (!tflAdapter?.servicesForStop || !nationalAdapter?.servicesForStops) throw new Error('TfL and national timetable adapters are required.');
  async function servicesForStops(stops, options = {}) {
    ...
    const national = await nationalAdapter.servicesForStops(stops, options);
    ...
    if (!successful.length && bods.length) { warnings.push(fallbackWarning); return sourceSuccess({ data: bods.map(service => ({ ...service, timetableSource: 'BODS fallback after TfL failure' })), warnings, provenance: { source: 'TfL scheduled timetable authority; BODS controlled fallback', authority: 'TfL', fallback: 'BODS', requestCount: requests.length, realtimeArrivalsUsed: false, anonymousRequest: true, apiKeyEmbedded: false } }); }
    ...
    return sourceSuccess({ data: composed, warnings, provenance: { source: 'TfL scheduled timetable authority; BODS controlled supplementary evidence', authority: 'TfL', requestCount: requests.length, successfulRequests: successful.length, realtimeArrivalsUsed: false, anonymousRequest: true, apiKeyEmbedded: false } });
  }
