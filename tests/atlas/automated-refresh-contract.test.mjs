...
const availability = fs.readFileSync(path.join(root, 'tools/atlas-data-publication/wait-for-publication.mjs'), 'utf8');
...
assert.match(workflow, /fetch-last-known-good\.mjs/);
assert.match(workflow, /measure_only:/);
assert.match(workflow, /Prepare and size-gate bounded reference-data publications/);
assert.match(workflow, /Publish validated Bus reference-data version/);
assert.match(workflow, /Publish validated TNDS reference-data version/);
assert.match(workflow, /publish-snapshot\.mjs/);
assert.doesNotMatch(workflow, /switch --orphan publication-reset/);
assert.match(workflow, /github\.ref == 'refs\/heads\/main' && inputs\.measure_only != true/);
assert.match(workflow, /Wait for both bounded Pages publications/);
assert.match(workflow, /Remove reference data from the application Pages payload/);
