...
const manifest = { schema: 'atlas-prepared-bus-data-v1', generatedAt: '2026-09-04T08:00:00Z', refreshAfterDays: 8, gridSize: 0.1, serviceShardKeyLength: 5, stopFields: ['id', 'naptanCode', 'name', 'indicator', 'direction', 'latitude', 'longitude', 'stopType', 'busStopType', 'locality', 'parentLocality', 'areaCode', 'modifiedAt', 'routes'], sources: { naptan: { url: 'https://official.example/naptan', downloadedAt: '2026-09-04T08:00:00Z', sha256: 'naptan-v1' }, bods: { url: 'https://official.example/bods', sha256: 'bods-v1', regions: ['south-east'] } }, stopShards: { g516_m1: 'stops/g516_m1.json.gz' }, serviceShards: { '2100A': ['services/2100A-south-east.json.gz'] }, representativeDates: { monday: '2026-09-07' } };
const stop = { id: '2100A', naptanCode: 'hrtfixture', name: 'High Street', indicator: 'Stop A', direction: 'N', latitude: 51.6859, longitude: -0.0331, areaCode: '210', routes: ['10'] };
...
function fetchFixture(url) {
  ...
  const value = path.endsWith('/manifest.json') ? manifest : path.endsWith('/stops/g516_m1.json.gz') ? { schema: 'atlas-prepared-bus-data-v1', stops: [packedStop] } : path.endsWith('/services/2100A-south-east.json.gz') ? { schema: 'atlas-prepared-bus-data-v1', services: [service] } : null;
...
test('prepared NaPTAN lookup returns authoritative nearby records', async () => {
