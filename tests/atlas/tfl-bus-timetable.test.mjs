...
assert.equal(result.data[0].publicRouteOrigin, 'Full Route Origin');
assert.equal(result.data[0].publicRouteDestination, 'Clapham Common');
assert.equal(result.data[0].source.routeMetadataEvidence.provider, 'Transport for London Unified API');
assert.equal(result.data[0].source.routeMetadataEvidence.cacheStatus, 'miss');
assert.match(result.data[0].source.routeMetadataEvidence.endpoint, /\/Line\/322\/Route/);
assert.equal(result.data[0].source.timetableEvidence.cacheStatus, 'miss');
assert.equal(result.data[0].source.timetableEvidence.retrievedAt, result.provenance.retrievedAt);
...
const [endpointRow] = buildPlannerBusServiceSummaries([frequencySummary], [{ id: '490TEST003', name: 'Selected stop' }]);
assert.equal(endpointRow.destination, 'Clapham Common');
assert.equal(endpointRow.publicEndpointEvidence.destination.evidenceClass, 'authoritative-route-section');
assert.ok(endpointRow.publicEndpointEvidence.destination.sourceIds.includes(result.data[0].id));
...
assert.equal(distinctSummary.typicalFrequency.basis, 'frequency-band-range');
assert.equal(distinctSummary.typicalFrequency.valueText, 'Typically every ~5–10 mins', 'different same-stop bands remain an honest range rather than being collapsed');
