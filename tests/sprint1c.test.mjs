...
test('actual production new-stop pair creates one compound Bus network upload and imports five Routes',()=>{
  const selected=[{naptanCode:'NEW-A',stopName:'New Stop A',locality:'New Town',direction:'Northbound',pairedStopGroup:'NEW-PAIR',coordinates:{latitude:51.5,longitude:-0.1}},{naptanCode:'NEW-B',stopName:'New Stop B',locality:'New Town',direction:'Southbound',pairedStopGroup:'NEW-PAIR',coordinates:{latitude:51.501,longitude:-0.101}}],plan=W.siteResearchPlan({stops:selected,library:emptyLibrary});
  assert.equal(plan.uploadCount,1);assert.equal(plan.summary.busNetworkTasks,1);assert.equal(plan.summary.busRouteTasks,0);assert.equal(plan.summary.relationshipTasks,0);assert.equal(plan.packs[0].tasks[0].kind,'bus-network');
  const request=W.siteResearchRequest(plan,plan.packs[0],{toolkit:'TPT-2.7.1'}),routes=Array.from({length:5},(_,index)=>{const value=route(String(index+31),'Origin '+index,'Destination '+index);return{...value,canonicalRouteKey:W.routeKey(value)}}),numbers=routes.map(item=>item.routeNumber),stops=selected.map(item=>({...item,routesServed:numbers,sources:[source],retrievalDate:'2026-07-23',verificationStatus:'verified'})),relationships=routes.flatMap(record=>stops.map(item=>relationship(item.naptanCode,record))),completed={schema:'tpt-site-research-completed-v1',version:1,requestGroupId:request.requestGroupId,packId:request.packId,packSequence:request.packSequence,totalPackCount:request.totalPackCount,busNetworkResults:[{taskId:request.tasks[0].taskId,stopRecords:stops,routeRecords:routes,relationshipRecords:relationships}],railResults:[],busStopResults:[],busRouteResults:[],relationshipResults:[],unresolvedResults:[]},result=W.validateSiteCompleted(request,completed);
  assert.equal(result.accepted.busStops.length,2);assert.equal(result.accepted.busRoutes.length,5);assert.equal(result.accepted.relationships.length,10);assert.equal(result.packComplete,true);
  const replanned=W.siteResearchPlan({stops,routes,relationships,library:{rail:[],stops,routes,stopRoutes:relationships}});assert.equal(replanned.uploadCount,0);
});
test('combined actual selection and self-instructing real templates use one upload',()=>{
  const rails=['C01','C02','C03'].map(code=>({name:'Station '+code,mode:'National Rail',identifiers:{crs:code},coordinates:{lat:51.5,lon:-0.1}})),stops=[{naptanCode:'CB-A',stopName:'Combined A',pairedStopGroup:'CB',coordinates:{latitude:51.5,longitude:-0.1}},{naptanCode:'CB-B',stopName:'Combined B',pairedStopGroup:'CB',coordinates:{latitude:51.501,longitude:-0.101}}],plan=W.siteResearchPlan({rail:rails,stops,library:emptyLibrary}),request=W.siteResearchRequest(plan,plan.packs[0],{toolkit:'TPT-2.7.1'}),json=JSON.stringify(request);
  assert.equal(plan.uploadCount,1);assert.equal(plan.packs[0].kind,'combined');assert.equal(request.taskCounts.rail,3);assert.equal(request.taskCounts.busNetworks,1);assert.doesNotMatch(json,/COMPLETED_(?:RAIL|BUS|STOP)/);assert.match(json,/"frequencyTotal"/);assert.match(json,/"canonicalRouteKey"/);assert.match(json,/"relationshipRecords"/);assert.match(json,/Missing total TPH blocks completion/);assert.match(json,/number alone is never identity/);
});
test('unsafe compound additions are rejected while safe in-scope records survive',()=>{
  const selected={naptanCode:'SAFE-A',stopName:'Safe Stop',locality:'Safe Town',direction:'Northbound',coordinates:{latitude:51.5,longitude:-0.1}},plan=W.siteResearchPlan({stops:[selected],library:emptyLibrary}),request=W.siteResearchRequest(plan,plan.packs[0]),baseRoute=route('88','Safe Origin','Safe Destination'),safeRoute={...baseRoute,canonicalRouteKey:W.routeKey(baseRoute)},safeStop={...selected,routesServed:['88'],sources:[source],retrievalDate:'2026-07-23',verificationStatus:'verified'},safeLink=relationship('SAFE-A',safeRoute),unrelated={...safeStop,naptanCode:'OTHER'},completed={schema:'tpt-site-research-completed-v1',version:1,requestGroupId:request.requestGroupId,packId:request.packId,packSequence:request.packSequence,totalPackCount:request.totalPackCount,busNetworkResults:[{taskId:request.tasks[0].taskId,stopRecords:[safeStop,unrelated],routeRecords:[safeRoute,{...safeRoute}],relationshipRecords:[safeLink]}],unresolvedResults:[]},result=W.validateSiteCompleted(request,completed);
  assert.equal(result.accepted.busStops.length,1);assert.equal(result.accepted.busRoutes.length,1);assert.equal(result.accepted.relationships.length,1);assert.ok(result.failed.some(item=>/unrelated Bus Stop/.test(item.reason)));assert.ok(result.failed.some(item=>/duplicate canonical Route/.test(item.reason)));
});
test('Admin bulk Rail plan is one pack of ten and Planner navigation remains visible',()=>{
  const plan=W.railPlan(Array.from({length:10},(_,index)=>({name:'Bulk '+index,mode:'National Rail',identifiers:{crs:'B'+index}})),{bulkAdmin:true});assert.equal(plan.safeLimit,10);assert.equal(plan.batches.length,1);assert.equal(plan.batches[0].records.length,10);
  const dashboard=fs.readFileSync(new URL('../index.html',import.meta.url),'utf8'),railHtml=fs.readFileSync(new URL('../modules/railway/index.html',import.meta.url),'utf8'),busHtml=fs.readFileSync(new URL('../modules/bus/index.html',import.meta.url),'utf8');assert.match(dashboard,/site-research/);assert.match(railHtml,/Open Combined Site Research/);assert.match(railHtml,/Use Railway-only research/);assert.match(railHtml,/bulkRailResearch/);assert.match(busHtml,/Open Combined Site Research/);assert.match(busHtml,/Use Bus-only research/);assert.match(railHtml,/admin-only[^>]*><h2>Admin Tools — Bulk/);
});
...
test('existing complete Route at a new stop creates Relationship-only research',()=>{
  ...
  assert.equal(plan.summary.busRouteTasks,0);assert.equal(plan.summary.relationshipTasks,1);assert.equal(plan.uploadCount,1);const request=W.siteResearchRequest(plan,plan.packs[0]);assert.equal(request.lockedSharedLibraryContext[W.routeKey(knownRoute)].routeNumber,'241');
});
